...
describe('Professional summary endpoint', () => {
  ...
  beforeAll(async () => {
    ...
    // create a patient user to be the owner of the appointment/payment
    const patientEmail = `patient-${unique}@test.local`;
    const patient = await prisma.usuario.create({ data: { nome: 'Paciente Test', email: patientEmail, cpf: `33333333333${unique}`, telefone: '11966666666', senha: 'h', tipo: 'paciente' } });
  ...
  afterAll(async () => {
    ...
    // remove users created for this spec
